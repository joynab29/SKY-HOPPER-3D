...
def get_time_t():
    return clamp01(timeElapsed / duration)
...
#Game restart
def restart_game():
    global jibon_shesh,lives, start_time, timeElapsed, last_time
    global player_x,player_y, player_z, player_vz, onGround
    global run_charge,ground_platform, jumps_left
    global jump_vx, jump_vy,space_was_down,keys_down
    global last_safeX, last_safeY,last_safeZ
    global player_dir
    ...
    last_safeX, last_safeY, last_safeZ = startX, startY, startZ
    camera_angle= 0.0
    camera_radius= 200
...
def draw_coin(c):
    ...
    t=timeElapsed* coin_Speed  # freezes when paused
...
def draw_sky_gradient(): #sky+star
    t = get_time_t()
...
def draw_stars():
    nighttt = get_night_factor()
    if nighttt <= 0.01:
        return

    brighttt = 0.25 + 0.75 * nighttt

    for (x, y, z, size) in stars:
        glPushMatrix()
        glTranslatef(x, y, z)
        glColor3f(brighttt, brighttt, brighttt)
...
def setupCamera():
    ...
    if first_person:
        ...
        forwardX = -math.sin(ang)
        forwardY = math.cos(ang)

        mathaheight = 85.0
        eyeX = px + forwardX * 12.0
        eyeY = py + forwardY * 12.0
        eyeZ = pz + mathaheight
    
        lookDist = 220.0
        lookX = eyeX + forwardX * lookDist
        lookY = eyeY + forwardY * lookDist
        lookZ = eyeZ - 60.0

        fp_camera_pos = (eyeX, eyeY, eyeZ)
...
def idle(): #main-loop
    ...
    global lives, jibon_shesh, timeElapsed, start_time
    global last_safeX, last_safeY, last_safeZ
    global last_time
    global camera_pos
    global jitse
    ...
    timeElapsed = now - start_time - paused_time_total
    if timeElapsed < 0:
        timeElapsed = 0.0
    ...
    steer_speed = base_speed * (air_control if not onGround else 1.0)
    ...
    player_x += (forwardX * move_f + rightX * move_s) * steer_speed * dt
    player_y += (forwardY * move_f + rightY * move_s) * steer_speed * dt
    ...
    if space_is_down and not space_was_down and jumps_left > 0:
        ...
        onGround = False
        jumps_left -= 1
        jump_vx = forwardX * base_speed * move_f
        jump_vy = forwardY * base_speed * move_f
    ...
    if top_z is not None:
        ...
        last_safeX, last_safeY, last_safeZ = player_x, player_y, player_z
        jumps_left = max_jump
        jump_vx = jump_vy = 0.0
    else:
        onGround = False
    ...
    if onGround and ground_platform and ground_platform["type"] == "moving":
        player_x += ground_platform["dx"]
        last_safeX += ground_platform["dx"]
    ...
    if player_z < -600:
        ...
        if lives <= 0:
            ...
            player_x, player_y, player_z = last_safeX, last_safeY, last_safeZ
            player_vz = 0.0
            onGround = True
            run_charge = 0.0
            jumps_left = max_jump
...
def draw_minimap(): #draw minimap
    ...
    glColor3f(*minimap_bg) #minimap bg
    glBegin(GL_QUADS)
...
def showScreen():
    ...
    time_left = max(0, int(duration - timeElapsed))
    mm = time_left // 60
    ss = time_left % 60
