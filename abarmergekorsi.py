...
def draw_door_on_platform(p):
    """
    Draws a simple door on top of the given platform
    """
    glPushMatrix()

    # Position door at center of the platform, slightly above it
    glTranslatef(
        p["x"],
        p["y"],
        p["z"] + platform_height
...
def specialKeyListener(key, x, y):
    ...
    # (optional) lock 3rd-person controls in first-person
    if first_person:
        return
...
def setupCamera():
    ...
    if first_person:
        # player view based on WORLD position (includes offset)
        px, py, pz = player_world_pos()
...
def draw_pause_icon():
    ...
    glColor3f(1.0, 1.0, 0.0)  # yellow icon

    if not paused:
        # PAUSE ||  (30px height, moved down)
        glBegin(GL_QUADS)
...
def idle():
    ...
    if platforms:
        ...
        if (
            abs(px - door_x) < 40 and
            abs(py - door_y) < 40 and
            abs(pz - door_z) < 70
        ):
            game_over = True
            game_won = True
